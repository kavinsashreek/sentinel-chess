...
const LESSONS = [
    { title: "Lesson 1: The Rook", desc: "Rooks move in straight lines. Click your Rook and capture the enemy pawn.", targetMoves: [{sx:4, sy:4, ex:4, ey:1}], setup: (g) => { g.setCustomPiece(4, 4, 3, 0); g.setCustomPiece(4, 1, 0, 1); g.forceTurn(0); } },
    ...
    { title: "Lesson 3: The Knight Fork", desc: "Use your Knight to attack two high-value pieces at once.", targetMoves: [{sx:3, sy:4, ex:2, ey:2}], setup: (g) => { g.setCustomPiece(3, 4, 1, 0); g.setCustomPiece(1, 2, 5, 1); g.setCustomPiece(4, 1, 3, 1); g.forceTurn(0); } }
];
...
async function init() {
    ...
    myNickname = localStorage.getItem('sentinel_nick_secure') || '';
    UI.nickInput.value = myNickname;
    UI.myDisplayName.innerText = myNickname || "You";
...
function setupEventListeners() {
    ...
    socket.on('profile_success', (approvedName) => {
    ...
    document.getElementById('btnToggle3D').addEventListener('click', () => {
        is3D = !is3D;
        if(is3D) UI.board.classList.add('board-3d');
        else UI.board.classList.remove('board-3d');
    });
    ...
    document.getElementById('btnAddFriend').addEventListener('click', () => {
        ...
        if (fId && fId !== myId && fId !== myNickname) {
            ...
            document.getElementById('friendIdInput').value = ''; loadFriendsList();
        }
    });
    ...
    document.querySelectorAll('.btn-track').forEach(btn => { btn.addEventListener('click', (e) => { learnProgress = parseInt(e.target.dataset.track); localStorage.setItem('sentinel_learn', learnProgress); localStorage.setItem('sentinel_learn_track_set', 'true'); learnTrackSet = true; renderLearnTab(); }); });
    document.getElementById('btnResetTrack').addEventListener('click', () => { learnTrackSet = false; localStorage.removeItem('sentinel_learn_track_set'); renderLearnTab(); });
    ...
    document.getElementById('recapButtons').addEventListener('click', (e) => { if(e.target.tagName === 'BUTTON') loadScenario(parseInt(e.target.dataset.lesson)); });

    document.getElementById('btnResign').addEventListener('click', () => { if (mode === 'LEARN') document.getElementById('btnReturnHome').click(); else triggerEndgame(myColor === 0 ? 1 : 0, "Resignation"); });
    ...
    document.getElementById('btnReturnHome').addEventListener('click', () => {
        ...
        document.getElementById('btnRandomMatch').innerText = "Random Match"; showPage(mode === 'LEARN' ? 'learnPage' : 'homePage'); loadHistoryList(); renderLearnTab();
    });
    ...
    UI.hintBtn.addEventListener('click', () => {
        ...
        if (hintsRemaining <= 0) { alert("No hints remaining!"); return; }
...
function showPage(pageId) { document.querySelectorAll('.page-container').forEach(p => p.classList.remove('active-page', 'hidden')); document.getElementById(pageId).classList.add('active-page'); }

function renderLearnTab() {
    document.getElementById('learnOnboardingUI').classList.add('hidden'); document.getElementById('learnRecapUI').classList.add('hidden'); document.getElementById('learnMainUI').classList.add('hidden');
    if (!learnTrackSet) { document.getElementById('learnOnboardingUI').classList.remove('hidden'); return; }
    if (learnProgress >= LESSONS.length) { document.getElementById('learnRecapUI').classList.remove('hidden'); document.getElementById('btnSkipRecap').innerText = "Curriculum Complete!"; document.getElementById('btnSkipRecap').disabled = true; } 
    else { if (learnProgress > 0) document.getElementById('learnRecapUI').classList.remove('hidden'); else { document.getElementById('learnMainUI').classList.remove('hidden'); document.getElementById('lessonTitleDisplay').innerText = LESSONS[learnProgress].title; document.getElementById('lessonDescription').innerText = LESSONS[learnProgress].desc; } }
    const recapBtns = document.getElementById('recapButtons'); recapBtns.innerHTML = '';
    for(let i = 0; i < Math.min(learnProgress, LESSONS.length); i++) recapBtns.innerHTML += `<button class="action-btn secondary hover-lift" data-lesson="${i}">Review: ${LESSONS[i].title}</button>`;
}
...
function loadScenario(index) {
    ...
    UI.hintBtn.classList.add('hidden'); document.getElementById('btnNextReplay').classList.add('hidden'); document.getElementById('btnResign').innerText = "Exit Lesson"; document.getElementById('btnResign').classList.remove('hidden');
    document.getElementById('capturedMe').innerHTML = ''; document.getElementById('capturedOpponent').innerHTML = '';
    updateBoardFromEngine(); showPage('gamePage'); UI.status.innerText = "Solve the Puzzle";
}
...
function verifyLearnMove(startX, startY, endX, endY) {
    ...
    if (startX === targetMove.sx && startY === targetMove.sy && endX === targetMove.ex && endY === targetMove.ey) {
        ...
        setTimeout(() => { UI.alert.classList.add('hidden'); if (learnProgress < LESSONS.length - 1) { learnProgress++; localStorage.setItem('sentinel_learn', learnProgress); loadScenario(learnProgress); } else { learnProgress = LESSONS.length; localStorage.setItem('sentinel_learn', learnProgress); document.getElementById('btnReturnHome').click(); } }, 1500);
    } else {
        UI.alert.innerText = "TRY AGAIN"; UI.alert.classList.remove('hidden'); setTimeout(() => { UI.alert.classList.add('hidden'); loadScenario(Math.min(learnProgress, LESSONS.length - 1)); }, 1500);
    }
}
...
function startMatch() {
    ...
    UI.instruction.classList.add('hidden'); UI.hintBtn.classList.remove('hidden'); document.getElementById('btnResign').innerText = "Resign"; document.getElementById('btnResign').classList.remove('hidden'); UI.analysis.classList.add('hidden');
    document.getElementById('capturedMe').innerHTML = ''; document.getElementById('capturedOpponent').innerHTML = '';
...
function addCapturedPiece(type, color) {
    ...
    if (color !== myColor) document.getElementById('capturedMe').appendChild(img);
    else document.getElementById('capturedOpponent').appendChild(img);
}

function updateStatusText() {
    if (mode === 'MULTIPLAYER') UI.status.innerText = game.getCurrentTurn() === myColor ? "Your Turn" : "Opponent Thinking...";
    else if (mode === 'REPLAY') UI.status.innerText = "Watching Replay - Analysis Mode"; else if (mode === 'LEARN') return; 
    ...
    if (selectedSquare === null) {
        if (game.getPieceColorAt(x, y) === game.getCurrentTurn()) {
            ...
            for (let i = 0; i < moves.size(); i += 2) {
                const tx = moves.get(i), ty = moves.get(i + 1); const sq = document.querySelector(`.square[data-x="${tx}"][data-y="${ty}"]`);
        ...
        if (state === 2) triggerEndgame(game.getCurrentTurn() === 0 ? 1 : 0, "Checkmate"); else if (state === 3) triggerEndgame(-1, "Stalemate");
            ...
            if (isSelfInitiated && mode === 'MULTIPLAYER') socket.emit('make_move', { room: roomName, startX, startY, endX, endY });
                ...
                setTimeout(() => { 
                    ...
                    if (aiSX !== -1) {
                        const aiTargetType = game.getPieceTypeAt(aiEX, aiEY); const aiTargetColor = game.getPieceColorAt(aiEX, aiEY);
                        const aiCapture = aiTargetColor !== -1;
                        ...
                        if(aiCapture) { SOUNDS.capture.play(); addCapturedPiece(aiTargetType, aiTargetColor); } else SOUNDS.move.play();
                    }
...
function triggerEndgame(winnerColor, reason) {
    ...
    const screen = document.getElementById('endgameScreen'); const title = document.getElementById('endgameTitle'); screen.className = 'endgame-overlay'; 
    if (winnerColor === -1) { screen.classList.add('draw'); title.innerText = "DRAW"; } else if (winnerColor === myColor) { screen.classList.add('win'); title.innerText = "YOU WON!"; } else { screen.classList.add('loss'); title.innerText = "YOU LOST"; }
...
function loadFriendsList() {
    ...
    else { friends.forEach(f => { const li = document.createElement('li'); li.innerHTML = `<span><b>${f}</b></span>`; const btn = document.createElement('button'); btn.innerText = "Challenge"; btn.dataset.id = f; li.appendChild(btn); list.appendChild(li); }); }
}
...
function loadHistoryList() {
    ...
    else { history.forEach((m, i) => { let c = m.result === 'Won' ? 'color:#3fb950' : (m.result === 'Lost' ? 'color:#f85149' : 'color:#8b949e'); const li = document.createElement('li'); li.innerHTML = `<div><strong style="${c}">${m.result}</strong> vs ${m.opponent}<br><small style="color:#8b949e">${m.date}</small></div>`; const btn = document.createElement('button'); btn.innerText = "Analyze"; btn.dataset.index = i; li.appendChild(btn); list.appendChild(li); }); }
}
...
function showReviewScreen() {
    ...
    const mot = document.getElementById('reviewMotivation'); if (accuracy >= 85) mot.innerText = "Brilliant play! You controlled the board like a Grandmaster."; else if (accuracy >= 60) mot.innerText = "Solid game! A few inaccuracies, but good overall structure."; else mot.innerText = "Tough game. Study the blunders shown in the replay to crush it next time!";
